...
var (
	...
	isPostgreSQLTestOptional = flag.Bool("is_pg_test_optional", true, "Boolean value to control whether the PostgreSQL test is optional")
...
func TestMain(m *testing.M) {
	...
	if err != nil {
		...
		klog.Fatalf("Failed to create PostgreSQL pool: %v", err)
	}
	defer pool.Close()
...
func initDatabaseSchema(ctx context.Context) {
	dropTablesSQL := "DROP TABLE IF EXISTS Checkpoint, Subtree, TiledLeaves, TreeState, Tessera"
	...
	if _, err := testPool.Exec(ctx, dropTablesSQL); err != nil {
		klog.Fatalf("Failed to drop all tables: %v", err)
	}

	if _, err := testPool.Exec(ctx, string(rawSchema)); err != nil {
...
func TestGetTile(t *testing.T) {
	ctx := t.Context()
	addFn, shutdown, r := newTestPostgreSQLStorage(t, ctx)
	defer func() {
		if err := shutdown(ctx); err != nil {
			t.Errorf("shutdown: %v", err)
		}
...
func TestReadMissingTile(t *testing.T) {
	ctx := t.Context()
	_, shutdown, r := newTestPostgreSQLStorage(t, ctx)
	defer func() {
		if err := shutdown(ctx); err != nil {
			t.Errorf("shutdown: %v", err)
		}
...
func TestParallelAdd(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	addFn, shutdown, _ := newTestPostgreSQLStorage(t, ctx)
	defer func() {
		if err := shutdown(ctx); err != nil {
			t.Errorf("shutdown: %v", err)
		}
...
func newTestPostgreSQLStorage(t *testing.T, ctx context.Context) (tessera.AddFn, func(context.Context) error, tessera.LogReader) {
	t.Helper()
	initDatabaseSchema(ctx)

	s, err := New(ctx, testPool)
	if err != nil {
